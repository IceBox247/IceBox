 ...
 */
/**
 * WalletConnect connect + sign using WC's own modal, which lists EVERY supported
 * wallet (not just MetaMask/Trust). The modal handles the deep-linking to
 * whichever wallet the user picks. One session: connect, fetch the nonce for the
 * returned address, sign, disconnect.
 */
export async function connectWalletConnect(
  ...
  const provider = await EthereumProvider.init({
    ...
    optionalChains: [56],
    showQrModal: true, // WC modal lists all wallets + handles the deep link
    metadata: {
      name: 'IceBox',
